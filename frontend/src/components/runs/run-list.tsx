...
const RunList: FC<RunListProps> = ({
  ...
  <ul
    ...
    className={cn(
      "stagger-rows divide-y divide-border",
      !flat && "overflow-hidden rounded-xl border border-border bg-surface-panel",
    )}
  >
    {runs.map((run) => {
      ...
      return (
        ...
        <li key={id}>
          <RowLink
            href={`/app/runs/${id}`}
            label={label ?? `Run ${shortHash(id)}`}
            selected={isSelected}
            className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:gap-4"
          >
            {onToggle ? (
              <label
                className={`relative z-10 flex shrink-0 items-center gap-2 text-xs ${
                  blocked && !isSelected
