...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-424-009966" alt="424 tests">
</p>
...
Point it at a recorded sequence. It tracks features through the images and solves the camera motion, fusing the IMU to recover metric scale if you ask it to. The result is scored against hardware-measured ground truth. The viewer overlays estimate on truth and the inspector holds the frame where a run went wrong, so a parameter change is judged by its effect on drift rather than by eye.
...
**Runs ranked by accuracy, across every sequence.** Sort by best ATE to see which config actually won, and filter to the failures when one did not. A run that was never scored reads "not scored" rather than zero, which would otherwise sort to the top as the best result.
...
1. A sequence is registered by path. The folder itself decides the frame count, the calibration and whether ground truth exists, so a sequence cannot be described as something it is not.
2. A run is queued and returns immediately. Estimating 2,821 frames takes about a minute, which is not something to hold a request open for.
3. A worker picks the run up and tracks features from frame to frame.
...
| Worker | Runs the queue, one estimate at a time |
...
Then get a sequence. driftline reads the ASL folder layout, so any TUM VI or EuRoC sequence
works. room1 is the one every figure above came from, 1.7 GB:

```bash
curl -O https://cdn3.vision.in.tum.de/tumvi/exported/euroc/512_16/dataset-room1_512_16.tar
tar -xf dataset-room1_512_16.tar -C data/
```

Register `data/dataset-room1_512_16` from the Datasets screen. Prefer a `room` sequence if you
want scores: those carry mocap ground truth for the whole trajectory. Anything else estimates
fine but has nothing to be scored against.
...
ATE and RPE are checked against [evo](https://github.com/MichaelGrupp/evo), so a scoring change that agrees with itself but disagrees with the reference implementation fails.
